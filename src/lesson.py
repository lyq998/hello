...
def login(first_flag=False):
    ...
    while True:
        try:
            ...
            # 再访问一次加入cookies
            r = s.get("http://" + jwc_ip +
                      "/loginAction.do", timeout=time_delay)
            # 登录
            r = s.post("http://" + jwc_ip + "/loginAction.do",
                       data=data, timeout=time_delay)
            ...
            # 检查密码是否正确
            r = s.get("http://" + jwc_ip +
                      "/xkAction.do?actionType=6", timeout=time_delay)
...
def xk(param):
    ...
    while True:
        try:
            # 查找课程
            r = s.get("http://" + jwc_ip +
                      "/xkAction.do?actionType=-1", timeout=time_delay)
            r = s.get("http://" + jwc_ip +
                      "/xkAction.do?actionType=5&pageNumber=-1&cx=ori", timeout=time_delay)
            r = s.post("http://" + jwc_ip + "/xkAction.do",
                       data=tparam, timeout=time_delay)
            # 选课
            r = s.post("http://" + jwc_ip + "/xkAction.do",
                       data=param, timeout=time_delay)
